...
int main() //main function
{
	...
	for (int i = 1; i <= junNum; i++) { 
		...
		for (map<int, vector<beacon>>::iterator itc = endTen.begin(); itc != endTen.end(); itc++) {
			tempTable = kalman.kalmanPrediction(itc->second, measurement);  //create probability table for each pair of end/start trace segments
...
/*
		...
		{
			if (probab[mm].probability < 0.1) //filter out the segment pair which probability too low (here < 0.1)
			{
...
*/
		...
		int nn = 0;
		int nnsize = probab.size();
		int maxP = 0;
		while (nnsize > 0){
			float tempro = probab[0].probability;
